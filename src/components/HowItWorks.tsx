  ...
  return (
    <section className="bg-black text-white pt-20 overflow-hidden relative">
        ...
        >
          ...
          {[...scrollTexts, ...scrollTexts].map((text, idx) => (
            <SwiperSlide key={idx} className="!w-auto">
              <span className="text-[#e5ff00] font-black text-sm md:text-lg tracking-[0.2em] uppercase whitespace-nowrap">
                {text}
              </span>
            </SwiperSlide>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 container mx-auto">
        {/* Solution Box */}
        <div className="bg-[#e5ff00] p-4 md:p-24 flex flex-col justify-center items-center text-center">
          ...
          <div className="space-y-8">
            {steps.map((step) => (
              <div key={step.id} className="flex items-start gap-6">
                <div className="bg-[#e5ff00] text-black w-10 h-10 flex items-center justify-center font-bold shrink-0">
                  {step.id}
                </div>
                <div>
